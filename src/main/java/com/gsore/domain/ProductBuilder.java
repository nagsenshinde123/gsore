...
public class ProductBuilder {
	...
	public Product build() {
		...
		if (this.sizes != null && !this.sizes.isEmpty()) {
			Set<RamSize> sizeElements = new HashSet<>();
			for (String val : this.sizes) {
				sizeElements.add(new RamSize(val,article));
			}	
			article.setSizes(sizeElements);
		}
		
		if (this.categories != null && !this.categories.isEmpty() ) {
			Set<MobileColor> catElements = new HashSet<>();
			for (String val : this.categories) {
				catElements.add(new MobileColor(val,article));
			}
			article.setCategories(catElements);
		}		
		if (this.brands != null && !this.brands.isEmpty() ) {
			Set<StorageSize> brandlements = new HashSet<>();
			for (String val : this.brands) {
				brandlements.add(new StorageSize(val,article));
			}
