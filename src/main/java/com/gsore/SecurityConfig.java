...
public class SecurityConfig extends WebSecurityConfigurerAdapter {
	...
	private BCryptPasswordEncoder passwordEncoder() {
		return PasswordSecurityUtility.passwordEncoder();
	}
	...
	protected void configure(HttpSecurity http) throws Exception {
		http.authorizeRequests().antMatchers(PUBLIC_MATCHERS).permitAll().antMatchers("/article/**").hasRole("ADMIN")
		...
		http.csrf().disable().cors().disable().formLogin().failureUrl("/login?error").loginPage("/login").permitAll()
				...
				.deleteCookies("remember-me").permitAll().and().rememberMe().key("aSecretKey");
	}
